...
- [ ] 👤 **M18 — Discord "is typing…" indicator.** After a bridge rebuild, DM an
  ...
  **Operator's, and it cannot be otherwise**: the indicator is a transient piece of Discord UI with no
  server-side record — there is nothing to query afterwards, so someone has to be looking at the client
  while the turn runs. The refresh interval and the clear-on-reply are asserted in the suite; whether a
  human sees it within a second is not something the suite can answer.
...
- [ ] 👤 **M23 — Auto-heal a failed adapter.** The mechanism is asserted by
  `adapter-supervisor.test.ts` (retry after the base backoff, exponential while failures continue,
  capped with half-jitter, cancelled on stop). What is not asserted is the live round trip, and it
  cannot be from here: it needs the network to Discord cut and restored on a **production** bridge two
  colleagues are using. That is a deliberate outage on someone else's working day, so it is the
  operator's call and his window.
